...
func TestPublicDelayAPI(t *testing.T) {
	t.Run("FixedDelay", func(t *testing.T) {
		gen := gtest.FixedDelay(100 * time.Millisecond)
		require.NotNil(t, gen)
		assert.Equal(t, 100*time.Millisecond, gen.Next())
	})

	t.Run("RangeDelay", func(t *testing.T) {
		gen := gtest.RangeDelay(50*time.Millisecond, 150*time.Millisecond)
		require.NotNil(t, gen)
		val := gen.Next()
	...
	t.Run("ExpoDelay", func(t *testing.T) {
		gen := gtest.ExpoDelay(200*time.Millisecond, 50*time.Millisecond, 500*time.Millisecond)
		require.NotNil(t, gen)
		val := gen.Next()
		assert.GreaterOrEqual(t, val, 50*time.Millisecond)
	...
	t.Run("GaussianDelay", func(t *testing.T) {
		gen := gtest.GaussianDelay(300*time.Millisecond, 50*time.Millisecond, 100*time.Millisecond, 500*time.Millisecond)
		require.NotNil(t, gen)
		val := gen.Next()
	...
	t.Run("NewDelayGenerator from Config", func(t *testing.T) {
		...
		gen, err := gtest.NewDelayGenerator(cfg)
