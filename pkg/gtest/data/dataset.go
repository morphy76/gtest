...
func LoadCSV(r io.Reader, strategy Strategy) (*DataSet, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("gtest/data: failed to read CSV header: %w", err)
	}
	...
	for {
		...
		if err != nil {
			return nil, fmt.Errorf("gtest/data: CSV parse error: %w", err)
		}
...
func LoadJSON(r io.Reader, strategy Strategy) (*DataSet, error) {
	...
	if err := decoder.Decode(&rawItems); err != nil {
		return nil, fmt.Errorf("gtest/data: JSON parse error: %w", err)
	}
...
func LoadJSONL(r io.Reader, strategy Strategy) (*DataSet, error) {
	...
	for scanner.Scan() {
		...
		if err := json.Unmarshal([]byte(line), &item); err != nil {
			return nil, fmt.Errorf("gtest/data: JSONL line parse error: %w", err)
		}
		records = append(records, parseObjectToRecord(item))
	}

	if err := scanner.Err(); err != nil {
		return nil, fmt.Errorf("gtest/data: JSONL scan error: %w", err)
	}
...
func LoadCSVFile(filePath string, strategy Strategy) (*DataSet, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("gtest/data: failed to open CSV file %q: %w", filePath, err)
	}
	defer func() {
		_ = f.Close()
	}()
...
func LoadJSONFile(filePath string, strategy Strategy) (*DataSet, error) {
	f, err := os.Open(filePath)
	if err != nil {
		return nil, fmt.Errorf("gtest/data: failed to open JSON file %q: %w", filePath, err)
	}
	defer func() {
		_ = f.Close()
	}()
...
func LoadJSONLFile(filePath string, strategy Strategy) (*DataSet, error) {
	f, err := os.Open(filePath)
	if err != nil {
		return nil, fmt.Errorf("gtest/data: failed to open JSONL file %q: %w", filePath, err)
	}
	defer func() {
		_ = f.Close()
	}()
